const pkRaw = `-----BEGIN PRIVATE KEY-----

-----END PRIVATE KEY-----`;

const b64 = pkRaw
    .replace('-----BEGIN PRIVATE KEY-----', '')
    .replace('-----END PRIVATE KEY-----', '')
    .replace(/\s/g, '');

const buf = Buffer.from(b64, 'base64');
console.log('Buffer length:', buf.length);
console.log('Start Byte:', buf[0].toString(16));

if (buf[0] === 0x30) {
    let lenByte = buf[1];
    if (lenByte & 0x80) {
        let n = lenByte & 0x7f;
        let declaredLen = 0;
        for (let i = 0; i < n; i++) {
            declaredLen = (declaredLen << 8) | buf[2 + i];
        }
        console.log('Declared ASN.1 Length:', declaredLen);
        console.log('Actual Body Length:', buf.length - (2 + n));
        if (declaredLen > buf.length - (2 + n)) {
            console.log(`❌ CRITICAL: The key is TRUNCATED. Missing ${declaredLen - (buf.length - (2 + n))} bytes.`);
        }
    }
}
